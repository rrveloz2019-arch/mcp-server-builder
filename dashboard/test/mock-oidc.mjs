...
export async function startMockOidc({ clientId, clientSecret }) {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (url.pathname === "/authorize") {
      ...
      codes.set(code, { user: state.nextUser, nonce: p.get("nonce"), challenge: p.get("code_challenge"), redirect: p.get("redirect_uri") });
